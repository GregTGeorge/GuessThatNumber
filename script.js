...
// Game restarts as longs as restartGame has a value of true
while (restartGame) {
    ...
    // Verifies the user's entry for a range number that is a number greater than zero (NOTE: All numbers, positive and negative, have a default boolean value of true, except for zerp which has a default boolean value of false. Also, NAN has a default boolean value of false.)
    ...
    guess = prompt(`Please eater a guess from 1 to ${rangeNum}. You have ${attempts} attempts left.`);
    ...
    while (true){
        ...
        if (guess === `Mellon`) {
            ...
            guess = prompt(`Please enter a guess rom 1 to ${rangeNum}. You have ${attempts} attempt(s) left:`);
        }

        // Tries to convert the user's guess into a number
        guess = parseInt(guess);

        // Verify the user's guess is a number greatee than zero and less than or equal to the range they set
        ...
        if (guess === randomNum){
            ...
            // Checks if user's guess was to low and prompts user to guess again if so
    ...
    // Loop continues until user submits a valiud response
    while (true){
        ...
        if (playAgain.toUpperCase() === `N`){
            ...
            restartGame = false;
            break;
            // Checks if the users answer is yes (AKA `Y` or `y`)
